...
//      System.out.println(chinaTown.get("자장면"));
//      chinaTown.keySet().stream().forEach(v -> System.out.println(v));
...
//      users.stream().map(u -> u.getAge()).forEach(u -> System.out.println(u));
...
//		datas.stream().sorted(Collections.reverseOrder()).forEach(System.out::println); //내림차순 정렬
