...
public class ATM {
	public static void main(String[] args) {
		// 은행 3개 고객 100명을 담을 Bank타입의2차원 배열 생성
...
//			은행을 선택한다.
			...
			while(true) {
				...
				if(choice == 1) { // 계좌 개설
					...
					user = arBank[index]; //사용자가 선택한 은행
					account = index + account; //계좌번호 앞에 사용자의 은행 붙여줌
					user.setAccount(account); //계좌번호 설정
					account = "";
					
					System.out.print("예금주: ");
					user.setName(sc.next()); //유저의 이름 입력
					
					while(true) { //핸드폰 번호 중복검사
